...
// Cache instances
const userCache = new AutoClearCache();
const pinAttemptCache = new AutoClearCache(CONFIG.PIN_ATTEMPT_WINDOW);
...
export const verifyPIN = async (req, res) => {
  try {
    ...
    const isMatch = await retryOperation(() =>
      bcrypt.compare(pin, user.transactionPIN),
    );
    ...
    if (error.message.includes("Too many PIN attempts")) {
...
export const updatePIN = async (req, res) => {
  try {
    ...
    console.error("Error in updatePIN:", error.message);
